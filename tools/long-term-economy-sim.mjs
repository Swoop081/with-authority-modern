...
import { grantBooster, openBooster, finalizePackUniversePoints, boosterCreditsFor, boosterEligible } from '../js/data/boosters.js';
...
const DAY=86400000;
// v0.13.7 release-calendar economy model. Future Season 1 subsets enter the
...
function openAll(p,r,stats,now){
 for(const setId of STORE_SET_ROTATION){
...
function simulate(kind,seed){
 ...
 for(let di=1;di<=90;di++){
  ...
  if(active){
   ...
   // Other matches.
   for(let i=0;i<cfg.otherMatches;i++) awardMatchSeasonXp(p,r()<cfg.winRate?'win':'loss');
   ...
   for(const tower of activeLiveEventTowers(now)){
    ...
    if(r()<prob){
      for(let m=0;m<5;m++){p.universePoints+=tower.winUp; stats.liveUp+=tower.winUp; stats.totalUpEarned+=tower.winUp; awardMatchSeasonXp(p,'win');}
      grantBooster(p,tower.clearBoosters,tower.event.rewardSetId);
    }
...
for(const kind of Object.keys(styles)){
 const reps=[]; for(let i=0;i<300;i++)reps.push(simulate(kind,1000+i*7919+kind.length*17));
 console.log('\n'+kind.toUpperCase());
