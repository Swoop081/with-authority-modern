...
test("v0.12.74 wrestler Specials are Very Rare", () => {
  const offenders = exclusiveCards.filter(card => card.kind === "special" && card.rarity !== 4);
  assert.deepEqual(offenders.map(card => [card.id, card.rarity]), []);
});
...
test("v0.12.74 rarity pass promotes signature examples without forcing generic utility upward", () => {
  ...
  assert.equal(byId.get("shoulder-up")?.rarity, 1, "generic utility Special remains outside the wrestler-exclusive rarity rule");
});
