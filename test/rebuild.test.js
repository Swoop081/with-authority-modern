...
import { superstars } from "../js/data/superstars.js?v=0.13.48";
import { decks } from "../js/data/decks.js?v=0.13.48";
import { allGameplayCards } from "../js/data/content.js?v=0.13.48";
import { collectionCards } from "../js/data/collection.js?v=0.13.48";
import { CARD_NUMBER_BY_ID } from "../js/data/card-number-manifest.js?v=0.13.48";
import { createProfile, migrateProfile, unlockSuperstar, addOwnedCard, addUniversePoints, totalOwnedCopies, cardOwnershipCap, hasSuperstar } from "../js/data/profile.js?v=0.13.48";
import { grantBooster, openBooster, finalizePackUniversePoints, boosterEligible } from "../js/data/boosters.js?v=0.13.48";
import { STORE_BOOSTER_PRICE, STORE_SUPERSTAR_PRICE, storeRotation, storeLeadOffCards, purchaseStoreBooster, purchaseStoreSuperstar } from "../js/data/store.js?v=0.13.48";
import { exhibitionOpponentIds, randomExhibitionOpponent } from "../js/data/matchmaking.js?v=0.13.48";
import { buildOwnedRecommendedDraft, autoFillOwnedDraft, recommendedDeckDraft, cardEligibilityForSuperstar, replaceLeadOffSlot, validateDeckDraft, selectedEntranceId, setSelectedEntrance, recommendedCategoryCounts, currentCategoryCounts } from "../js/data/deck-builder.js?v=0.13.48";
import { tierReward, claimSeasonTier, SEASON_1, SEASON_2_COMPLETION_SUPERSTAR } from "../js/data/seasons.js?v=0.13.48";
import { seasonExclusiveSuperstars } from "../js/data/season-exclusive.js?v=0.13.48";
import { season2GoldbergCards } from "../js/data/season2-goldberg-cards.js?v=0.13.48";
import { MatchEngine } from "../js/engine/MatchEngine.js?v=0.13.48";
import { moveEligibility, canPlayMomentum, canAttemptPin, canPlayAction } from "../js/engine/rules.js?v=0.13.48";
import { decisionOwner, cpuDecision, executeCpuDecision } from "../js/ai/WrestlingAI.js?v=0.13.48";
import { healthZone } from "../js/engine/health.js?v=0.13.48";
import { LAUNCH_LIVE_SET_IDS, isLaunchLiveSetId, isUnreleasedSetId } from "../js/data/release.js?v=0.13.48";
...
test("canonical collector manifest is gap-free and matches Collection plus Card Art Studio for every active card", async()=>{
  ...
  const { CARD_NUMBER_MANIFEST, CARD_NUMBER_BY_ID } = await import("../js/data/card-number-manifest.js?v=0.13.48");
...
test("starting HP roster uses the locked durability spread",()=>{
  ...
  assert.deepEqual(values,[57,58,59,60,61,62,63,64,65,66,67,68,69,70,72]);
...
test("v0.12.33 keeps every Superstar on the no-cap tuned durability model",()=>{
  ...
  assert.ok(stars.every(star=>Number.isInteger(star.hp)&&star.hp>=55),"all 52 Superstars use valid tuned starting HP values");
...
test("v0.12.12 public game exposes only the three launch series",()=>{
  ...
  assert.equal(launchCards.length,315);
