...
import { superstars } from "../js/data/superstars.js?v=0.13.71";
import { decks } from "../js/data/decks.js?v=0.13.71";
import { allGameplayCards } from "../js/data/content.js?v=0.13.71";
import { collectionCards } from "../js/data/collection.js?v=0.13.71";
import { CARD_NUMBER_BY_ID } from "../js/data/card-number-manifest.js?v=0.13.71";
import { createProfile, migrateProfile, unlockSuperstar, addOwnedCard, addUniversePoints, totalOwnedCopies, cardOwnershipCap, hasSuperstar } from "../js/data/profile.js?v=0.13.71";
import { grantBooster, openBooster, finalizePackUniversePoints, boosterEligible } from "../js/data/boosters.js?v=0.13.71";
import { STORE_BOOSTER_PRICE, STORE_SUPERSTAR_PRICE, storeRotation, storeLeadOffCards, purchaseStoreBooster, purchaseStoreSuperstar } from "../js/data/store.js?v=0.13.71";
import { exhibitionOpponentIds, randomExhibitionOpponent } from "../js/data/matchmaking.js?v=0.13.71";
import { buildOwnedRecommendedDraft, autoFillOwnedDraft, recommendedDeckDraft, cardEligibilityForSuperstar, replaceLeadOffSlot, validateDeckDraft, selectedEntranceId, setSelectedEntrance, recommendedCategoryCounts, currentCategoryCounts } from "../js/data/deck-builder.js?v=0.13.71";
import { tierReward, claimSeasonTier, SEASON_1, SEASON_2_COMPLETION_SUPERSTAR } from "../js/data/seasons.js?v=0.13.71";
import { seasonExclusiveSuperstars } from "../js/data/season-exclusive.js?v=0.13.71";
import { season2GoldbergCards } from "../js/data/season2-goldberg-cards.js?v=0.13.71";
import { MatchEngine } from "../js/engine/MatchEngine.js?v=0.13.71";
import { moveEligibility, canPlayMomentum, canAttemptPin, canPlayAction } from "../js/engine/rules.js?v=0.13.71";
import { decisionOwner, cpuDecision, executeCpuDecision } from "../js/ai/WrestlingAI.js?v=0.13.71";
import { healthZone } from "../js/engine/health.js?v=0.13.71";
import { LAUNCH_LIVE_SET_IDS, isLaunchLiveSetId, isUnreleasedSetId } from "../js/data/release.js?v=0.13.71";
...
test("canonical collector manifest is gap-free and matches Collection plus Card Art Studio for every active card", async()=>{
  ...
  const { CARD_NUMBER_MANIFEST, CARD_NUMBER_BY_ID } = await import("../js/data/card-number-manifest.js?v=0.13.71");
...
test("v0.12.33 keeps every Superstar on the no-cap tuned durability model",()=>{
  assert.equal(stars.length,62);
  assert.ok(stars.every(star=>Number.isInteger(star.hp)&&star.hp>=55),"all 62 Superstars use valid tuned starting HP values");
