...
import { superstars } from "../js/data/superstars.js?v=0.12.57";
import { decks } from "../js/data/decks.js?v=0.12.57";
import { allGameplayCards } from "../js/data/content.js?v=0.12.57";
import { collectionCards } from "../js/data/collection.js?v=0.12.57";
import { CARD_NUMBER_BY_ID } from "../js/data/card-number-manifest.js?v=0.12.57";
import { createProfile, migrateProfile, unlockSuperstar, addOwnedCard, addUniversePoints, totalOwnedCopies, cardOwnershipCap, hasSuperstar } from "../js/data/profile.js?v=0.12.57";
import { grantBooster, openBooster, finalizePackUniversePoints, boosterEligible } from "../js/data/boosters.js?v=0.12.57";
import { STORE_BOOSTER_PRICE, STORE_SUPERSTAR_PRICE, storeRotation, storeLeadOffCards, purchaseStoreBooster, purchaseStoreSuperstar } from "../js/data/store.js?v=0.12.57";
import { exhibitionOpponentIds, randomExhibitionOpponent } from "../js/data/matchmaking.js?v=0.12.57";
import { buildOwnedRecommendedDraft, autoFillOwnedDraft, recommendedDeckDraft, cardEligibilityForSuperstar, replaceLeadOffSlot, validateDeckDraft, selectedEntranceId, setSelectedEntrance, recommendedCategoryCounts, currentCategoryCounts } from "../js/data/deck-builder.js?v=0.12.57";
import { tierReward, claimSeasonTier, SEASON_1, SEASON_2_COMPLETION_SUPERSTAR } from "../js/data/seasons.js?v=0.12.57";
import { seasonExclusiveSuperstars } from "../js/data/season-exclusive.js?v=0.12.57";
import { season2GoldbergCards } from "../js/data/season2-goldberg-cards.js?v=0.12.57";
import { MatchEngine } from "../js/engine/MatchEngine.js?v=0.12.57";
import { moveEligibility, canPlayMomentum, canAttemptPin, canPlayAction } from "../js/engine/rules.js?v=0.12.57";
import { decisionOwner, cpuDecision, executeCpuDecision } from "../js/ai/WrestlingAI.js?v=0.12.57";
import { healthZone } from "../js/engine/health.js?v=0.12.57";
import { LAUNCH_LIVE_SET_IDS, isLaunchLiveSetId, isUnreleasedSetId } from "../js/data/release.js?v=0.12.57";
...
test("Daily Store rotates SS to HOF to EVO, charges UP, blocks repurchase and grants only the Store unlock collection package",()=>{
  ...
  assert.ok(ownedDraft.length>=0&&ownedDraft.length<60);
...
test("Season milestone road now builds The Final Boss while preserving milestone Universe Points",()=>{
  assert.equal(tierReward(1).kind,'booster');
  assert.deepEqual(tierReward(5),{tier:5,kind:'final-boss-card',exclusive:true,cardId:'the-rock-final-boss-slap',name:'Final Boss Slap',amount:1,rewardType:'exclusive-move',label:'EXCLUSIVE MOVE',bonusUniversePoints:100});
  assert.equal(tierReward(10).cardId,'the-rock-rock-bottom');
  assert.equal(tierReward(20).cardId,'special-the-rock');
  assert.equal(tierReward(25).cardId,'people-championship');
  assert.equal(tierReward(30).cardId,'the-rock-people-s-elbow');
  assert.equal(tierReward(40).cardId,'entrance-the-rock');
  assert.equal(tierReward(50).cardId,'superstar-the-rock');
  assert.equal(tierReward(35).kind,'universe-points');
  assert.equal(tierReward(35).amount,200);
  const p=createProfile('cm-punk');
  p.seasons['season-1'].xp=500;
  const reward=claimSeasonTier(p,5);
  assert.equal(reward.cardId,'the-rock-final-boss-slap');
  assert.equal(p.ownedCards['the-rock-final-boss-slap']?.normal,1);
  assert.equal(p.universePoints,100,'card milestone keeps the old UP value as bonus currency');
});
...
test("canonical collector manifest is gap-free and matches Collection plus Card Art Studio for every active card", async()=>{
  ...
  const { CARD_NUMBER_MANIFEST, CARD_NUMBER_BY_ID } = await import("../js/data/card-number-manifest.js?v=0.12.57");
...
test("Chad Gable RAW Series 1 package is locked, playable and wired to Olympic Pedigree / Shoosh",()=>{
  ...
  assert.equal(chad.hp,61); assert.equal(chad.methodLimits.technical,null); assert.equal(chad.methodLimits.strength,4); assert.equal(chad.methodLimits.agility,2); assert.equal(chad.methodLimits.strike,0);
...
test("starting HP roster uses the locked durability spread",()=>{
  const expected = {"iyo-sky":58,"mankind":68,"the-rock":70,"hulk-hogan":68,"bayley":67,"cm-punk":63,"paige":64,"seth-rollins":64,"andre-the-giant":66,"stephanie-vaquer":66,"randy-savage":59,"roman-reigns":63,"charlotte-flair":65,"kevin-owens":62,"kane":69,"the-undertaker":65,"ultimate-warrior":67,"rhea-ripley":63,"cody-rhodes":63,"oba-femi":62,"stone-cold-steve-austin":66,"liv-morgan":67,"brock-lesnar":67,"gunther":65,"becky-lynch":64,"logan-paul":61,"sol-ruca":58,"chad-gable":61,"raquel-rodriguez":59,"rey-mysterio":60,"dominik-mysterio":59,"penta":62};
  ...
  assert.deepEqual(values,[58,59,60,61,62,63,64,65,66,67,68,69,70]);
  assert.equal(starById.get('the-rock').hp,Math.max(...stars.map(s=>s.hp)),'Final Boss remains the HP ceiling');
});
...
test("Raquel Rodriguez RAW Series 1 package is locked, playable, and all four RAW decks are exactly 60 pages",()=>{
  ...
  assert.equal(raquel.hp,59); assert.equal(raquel.methodLimits.strength,null); assert.equal(raquel.methodLimits.strike,3); assert.equal(raquel.methodLimits.agility,1); assert.equal(raquel.methodLimits.technical,0);
...
test("Dominik Mysterio Worlds Collide package is locked and playable",()=>{
  ...
  assert.equal(dom.hp,59); assert.deepEqual(dom.starterMomentum,{agility:7,strike:3,technical:2});
...
test("SmackDown Series 1 Danhausen package is locked and playable",()=>{
  const dan=starById.get('danhausen'); assert.ok(dan); assert.equal(dan.hp,66);
...
test("SmackDown Series 1 Tiffany Stratton package is locked and playable",()=>{
  const tiffany=starById.get('tiffany-stratton'); assert.ok(tiffany); assert.equal(tiffany.hp,60);
...
test("SmackDown Series 1 Chelsea Green package is locked and counter-control mechanics execute",()=>{
  const chelsea=starById.get('chelsea-green'); assert.ok(chelsea); assert.equal(chelsea.hp,59);
...
test("SmackDown Series 1 Damian Priest package is locked and punishment mechanics execute",()=>{
  const priest=starById.get('damian-priest'); assert.ok(priest); assert.equal(priest.hp,61);
...
test("canonical health bands are Green 65%+, Amber 25-64%, Red 0-24% and pins follow them",()=>{
  ...
  d.maxHp=100;
  d.hp=65; assert.equal(healthZone(d),'green'); assert.equal(canAttemptPin(s,'p1').legal,true,'any post-Move cover window may attempt the 5% flash pin'); assert.equal(g._pinChance('p1'),5);
  d.hp=24; assert.equal(g._pinChance('p1'),5,'16+ actual HP remains a 5% flash pin');
...
test("Survivor Series Series 1 Drew, Randy, Sami and Jacob packages are locked, numbered and 60 pages",()=>{
  const expected={
    ...
    'jacob-fatu':{hp:62,momentum:{strength:6,strike:3,agility:3},codes:['SVS1-025','SVS1-030']}
  };
...
test("Survivor Series Series 1 is complete with Solo Sikoa, Jade Cargill and Nia Jax",()=>{
  ...
  const expected={
    'solo-sikoa':{hp:61,momentum:{strike:6,strength:5,agility:1},card:'SVS1-040'},
    'jade-cargill':{hp:62,momentum:{strength:6,strike:4,agility:2},card:'SVS1-044'},
    'nia-jax':{hp:67,momentum:{strength:7,strike:4,agility:1},card:'SVS1-049'}
  };
  for(const [id,x] of Object.entries(expected)){
    const star=starById.get(id);assert.ok(star,id);assert.equal(star.hp,x.hp,id);assert.deepEqual(star.starterMomentum,x.momentum,id);
...
test("v0.12.12 public game exposes only the three launch series",()=>{
  ...
  assert.equal(launchCards.length,284);
