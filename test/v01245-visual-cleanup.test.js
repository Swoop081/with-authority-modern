...
test('Season keeps the hero before the compact daily booster control', () => {
  const seasons = functionSlice('renderSeasons', 'renderChallenges');
  assert.ok(seasons.indexOf('season-final-boss-hero') < seasons.indexOf('season-free-pack-cta'));
  assert.match(seasons, /season-free-pack-cta/);
  assert.match(seasons, /season-free-pack-button/);
