...
await import("../js/data/superstar-nameplates.js?v=0.13.74");
const profiles=globalThis.WWE_LEGACY_SUPERSTAR_NAMEPLATES;

test("v0.13.74 tier-up numerals scale by digit count without clipping",()=>{
...
test("v0.13.74 live-event detail spends available viewport on complete larger cards",()=>{
...
test("v0.13.74 booster vault centers pack art, quantity badge and empty state",()=>{
...
test("v0.13.74 nameplates retain unique identities with readability-first condensed stacks",()=>{
  assert.equal(Object.keys(profiles).length,62);
...
test("v0.13.74 matchup uses full phone viewport and distributes the five major bands",()=>{
