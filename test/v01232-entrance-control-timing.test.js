...
test('v0.12.32 Entrance Momentum is preloaded but Entrance Adrenaline waits for first Control',()=>{
  ...
  assert.equal(s.players.p1.adrenaline,2,'P1 receives Eighth Wonder Adrenaline with opening Control');
  ...
  assert.equal(s.players.p2.adrenaline,2,'P2 receives Entrance Adrenaline exactly when P2 first gains Control');

  assert.equal(g.passTurn('p2'),true);
  assert.equal(s.playerInControl,'p1');
  assert.equal(s.players.p1.adrenaline,2,'P1 Entrance Adrenaline does not retrigger');
  assert.equal(g.passTurn('p1'),true);
  assert.equal(s.players.p2.adrenaline,2,'P2 Entrance Adrenaline does not retrigger');
