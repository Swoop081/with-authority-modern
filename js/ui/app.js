import { assetUrl, BUILD_VERSION } from "../config/build.js?v=0.13.37";
import { fetchLatestBuild, isNewerBuild, updateNavigationUrl } from "../config/update.js?v=0.13.37";
import { superstars } from "../data/superstars.js?v=0.13.37";
import { decks } from "../data/decks.js?v=0.13.37";
import { sets } from "../data/sets.js?v=0.13.37";
import { playerReleasedCollectibleSetIds, isPlayerReleasedSetId, isPlayerVisibleSuperstar } from "../data/release.js?v=0.13.37";
import { collectionCards, setCollection, setCollections, cardsForSet } from "../data/collection.js?v=0.13.37";
import { artworkFor, superstarArtwork, menuSuperstarPhotoFor, finalBossRockMenuArtwork, superstarCardArtFor, superstarHeadshotFor, finishedCardArtFor, legacyFinishedCardArtFor, layeredCardArtFor } from "../data/artwork.js?v=0.13.37";
import { STARTER_CHOICES, createProfile, hasSuperstar, loadProfile, saveProfile, resetProfile, setDeckAssistance, ownedCount } from "../data/profile.js?v=0.13.37";
import { openBooster, openSuperPack, openLadderCompletionPack, openChampionshipPack, grantBooster, grantVictoryBooster, boosterCreditsFor, superPackCreditsFor, finalizePackUniversePoints } from "../data/boosters.js?v=0.13.37";
import { STORE_BOOSTER_PRICE, STORE_SUPERSTAR_PRICE, storeRotation, storeSuperstars, storeLeadOffCards, purchaseStoreBooster, purchaseStoreSuperstar } from "../data/store.js?v=0.13.37";
import { randomExhibitionOpponent } from "../data/matchmaking.js?v=0.13.37";
import { buildPlayableDeck, findPackUpgrades, applyUpgrade } from "../data/deck-assistant.js?v=0.13.37";
import { MatchEngine } from "../engine/MatchEngine.js?v=0.13.37";
import { canPlayMomentum, canPlayEntrance, canPlayAction, canPlaySupport, canPlayManager, canPlaySpecial, effectiveTotalMomentum, moveEligibility, canCounter, counterEligibility, autoCounterEligibility, autoCounterCost, canAttemptPin, canPlayPinEscape, submissionThreshold, canReturnToRing, canFollowOutside } from "../engine/rules.js?v=0.13.37";
import { totalMomentum } from "../engine/utils.js?v=0.13.37";
import { healthZone } from "../engine/health.js?v=0.13.37";
import { decisionOwner } from "../ai/WrestlingAI.js?v=0.13.37";
import { advanceCpuUntilHuman } from "./turn-driver.js?v=0.13.37";
import { reconstructCurrentPlayPile } from "./play-pile.js?v=0.13.37";
import { LADDER_LIVES, LADDER_LENGTH, ladderState, startLadderRun, currentLadderOpponent, recordLadderMatch } from "../data/ladder.js?v=0.13.37";
import { KING_OF_THE_RING_ROUNDS, kingOfTheRingState, startKingOfTheRing, currentKingOfTheRingOpponent, recordKingOfTheRingMatch, prepareKingOfTheRingReward, markKingOfTheRingCoronationSeen, claimKingOfTheRingReward, resetKingOfTheRing } from "../data/king-of-the-ring.js?v=0.13.37";
import { CHAMPIONSHIP_ROAD_LENGTH, CHAMPIONSHIP_STAGES, CHAMPIONSHIP_DIFFICULTY_ORDER, CHAMPIONSHIP_DIFFICULTIES, CHAMPIONSHIP_ROAD_SECTIONS, CHAMPIONSHIP_ROAD_OPPONENTS, championshipRoadState, championshipDifficultyUnlocked, championshipRoadDifficultyModifier, championshipRoadSectionForStage, startChampionshipRoad, currentChampionshipOpponent, recordChampionshipMatch, resetChampionshipRoad } from "../data/championship-road.js?v=0.13.37";
import { LIVE_EVENT_LENGTH, LIVE_EVENT_WIN_UP, LIVE_EVENT_CLEAR_BOOSTERS, activeLiveEventTowers, liveEventTowerByKey, liveEventTowerState, startLiveEventTower, currentLiveEventTowerOpponent, currentLiveEventTowerStage, recordLiveEventTowerMatch, liveEventRotation, liveEventStage, weeklyLiveEventState } from "../data/live-events.js?v=0.13.37";
import { challengeState, claimChallenge, recordCompletedMatchChallenges } from "../data/challenges.js?v=0.13.37";
import { CAREER_MODES, careerRecord, achievementProgress, recordCareerMatch, refreshCareerAchievements } from "../data/career.js?v=0.13.37";
import { setProgressState, collectionProgress, availableMilestoneRewards, claimMilestone } from "../data/set-progression.js?v=0.13.37";
import { MOVE_TYPE_LABELS } from "../data/move-types.js?v=0.13.37";
import { COUNTER_STATE_LABELS, SUBMISSION_TARGET_LABELS } from "../data/counter-states.js?v=0.13.37";
import { CATALOGUE_PAGE_SIZE, defaultCatalogueFilters, catalogueOptions, filterAndSortCatalogue, superstarIdsForCard, isSharedCard } from "../data/catalogue.js?v=0.13.37";
import { DECK_LAB_CATEGORIES, createDeckDraft, recommendedDeckDraft, optimizeDeck, aggregateDeck, eligibleOwnedCards, allOwnedEntrances, ownedCardsForCategory, addCardToDraft, removeCardFromDraft, replaceLeadOffSlot, validateDeckDraft, materializeDraft, leadOffIds, buildOwnedRecommendedDraft, buildBestOwnedRecommendedDraft, recommendedDeckComparison, recommendedEntranceId, recommendedDeckMissingCount, autoFillOwnedDraft, recommendedCategoryCounts, currentCategoryCounts, cardEligibilityForSuperstar, entranceEligibilityForSuperstar, selectedEntranceId, setSelectedEntrance, ownedTotal, categoryForCard } from "../data/deck-builder.js?v=0.13.37";
import { RECOMMENDED_DECK_SHAPE } from "../data/deck-health.js?v=0.13.37";
import { SEASON_1, SEASON_TIER_COUNT, XP_PER_TIER, MATCH_XP, seasonState, seasonTier, seasonLevelProgress, seasonTimeRemaining, nextRoadmapNode, roadmapNodeStatus, awardMatchSeasonXp, tierReward, claimSeasonTier, claimAllSeasonTiers, freePackStatus, claimFreeSeasonBooster } from "../data/seasons.js?v=0.13.37";
import { GAME_RULE_SECTIONS, PIN_CHANCE_TABLE, LIVE_EVENT_WEEK } from "../data/game-rules.js?v=0.13.37";
import { SAVE_FILENAME, exportSaveToFiles, readSaveFile, saveImportRollback, loadImportRollback, clearImportRollback, backupMetadata } from "../data/save-backup.js?v=0.13.37";
...
function openablePackCount(p) {
  ...
  return CURRENT_PLAYER_SET_IDS.reduce((sum, setId) => {
    ...
    const superPacks = superPackCreditsFor(p, setId);
    ...
    return sum + standard + superPacks + ladderRewards + championshipRewards;
  }, 0);
}
...
function finishUnlockCelebration(openDeckLab = false) {
  ...
  if (profile.pendingUnlockCelebrations.length) {
...
function processPack(kind = "standard") {
  try {
    ...
    lastPack = kind === "super" ? openSuperPack(profile, Math.random, activeBoosterSetId) : kind === "ladder" ? openLadderCompletionPack(profile, Math.random, activeBoosterSetId) : kind === "championship" ? openChampionshipPack(profile, Math.random, activeBoosterSetId) : openBooster(profile, Math.random, activeBoosterSetId);
...
function renderBoosters() {
  ...
  const superPacks=superPackCreditsFor(profile,activeBoosterSetId), ladderPacks=ladder.completionPackCreditsBySet?.[activeBoosterSetId]??0, championshipPacks=road.championshipPackCreditsBySet?.[activeBoosterSetId]??0;
  const packTitle=currentPackType==="super"?"SUPER PACK":currentPackType==="ladder"?"MONEY IN THE BANK":currentPackType==="championship"?"CHAMPIONSHIP ROAD":setInfo.name.toUpperCase();
  const packWrapperTitle=currentPackType==="standard"?"SERIES 1":"SUPER PACK";
  const packSubtitle=currentPackType==="super"?"BOOSTED ODDS · 1 FOIL · 1 RARE+":currentPackType==="ladder"?"LEGACY SUPER PACK · 1 FOIL · 1 RARE+":currentPackType==="championship"?"LEGACY SUPER PACK · 1 FOIL · 1 RARE+":"SERIES 1 · 5 CARDS · 1 GUARANTEED FOIL";
  ...
  const summaryCard = (p,index,slotClass="") => `
    <article class="pack-summary-card actual-card-summary ${slotClass} rarity-${p.card.rarity} ${p.foil?'is-foil':''} ${p.universePointsValue?'is-up-converted':''}">
      <div class="pack-summary-actual-card">${p.universePointsValue ? upRewardTile(p,"summary-up-reward",`data-booster-inspect="${index}"`) : collectibleCardMarkup(p.card,{flipped:false,foil:p.foil,extraClass:"pack-summary-ccg",flipAttr:`data-booster-inspect="${index}"`})}</div>
  ...
  if (packStage === "sealed") {
    ...
    const cardMarkup = converted
      ...
      : `<div class="booster-flip-card single-pack-card is-revealed is-current rarity-${p.card.rarity} ${p.foil?'is-foil':''} ${converting?'duplicate-disintegrating':''}">
          ${collectibleCardMarkup(p.card,{flipped:false,foil:p.foil,extraClass:"booster-ccg",flipAttr:`data-booster-next="${boosterFocusIndex}"`})}
    ...
    const moreCurrentPacks = currentPackType === "super" ? superPackCreditsFor(profile, activeBoosterSetId) : currentPackType === "ladder" ? (ladder.completionPackCreditsBySet?.[activeBoosterSetId] ?? 0) : currentPackType === "championship" ? (road.championshipPackCreditsBySet?.[activeBoosterSetId] ?? 0) : boosterCreditsFor(profile, activeBoosterSetId);
    ...
    packArea=`<section class="pack-summary-screen premium-pack-summary">
      ...
      <div class="pack-summary-top-actions two-button-row">${canOpenAnotherPack?'<button id="open-another-pack" class="primary">OPEN ANOTHER PACK</button>':'<button id="review-pack-upgrades-top" class="primary">REVIEW UPGRADES</button>'}<button id="finish-pack-top" class="nav-button">${packReturnLabel}</button></div>
      ...
      <div class="pack-summary-actions two-button-row"><button id="review-pack-upgrades" class="start-match">Review Roster & Deck Upgrades</button><button id="finish-pack-summary" class="nav-button">Done</button></div>
    </section>`;
    ...
    <section class="upgrade-panel booster-modal-upgrades">
      ...
      ${appliedPackUpgrades.length ? `<div class="applied-upgrade-list">${appliedPackUpgrades.map(u=>`<article class="upgrade-row applied-upgrade-row"><div class="upgrade-pull-thumb">${cardArtFace(u.pull.card)}</div><div><b>${superstarById[u.superstarId]?.name}</b><span><strong>APPLIED</strong> · ${u.type==='foil-preference'?`USE ${u.addName}`:u.type==='entrance'?`EQUIP ${u.addName} · REPLACE ${u.removeName}`:`ADD ${u.addName} · REMOVE ${u.removeName}`}</span><small>${u.reason}</small></div></article>`).join('')}</div>` : ''}
      ${pendingUpgrades.length ? pendingUpgrades.map((u,i)=>`<article class="upgrade-row">
        ...
        <div><b>${superstarById[u.superstarId]?.name}</b><span><strong>${u.type==='foil-preference'?'FOIL PREFERENCE':u.type==='entrance'?'ENTRANCE UPGRADE':'RECOMMENDED BUILD'}</strong> · ${u.type==='foil-preference'?`USE ${u.addName}`:u.type==='entrance'?`EQUIP ${u.addName} · REPLACE ${u.removeName}`:`ADD ${u.addName} · REMOVE ${u.removeName}`}</span><small>${u.reason}</small></div>
        ${manual?'':`<div><button data-accept-upgrade="${i}" class="primary">${u.type==='foil-preference'?'Use Foil':u.type==='entrance'?'Equip Entrance':'Upgrade Deck'}</button><button data-decline-upgrade="${i}" class="secondary">Keep As-Is</button></div>`}
  ...
  for (const vaultSet of vaultSets) {
    ...
    const superCount = superPackCreditsFor(profile, vaultSet.id);
    ...
    if (superCount > 0) vaultBuckets.push({ setId:vaultSet.id, type:"super", count:superCount, label:"SUPER PACK", subtitle:"BOOSTED ODDS · 1 FOIL · 1 RARE+" });
    if (ladderCount > 0) vaultBuckets.push({ setId:vaultSet.id, type:"ladder", count:ladderCount, label:"LEGACY SUPER PACK", subtitle:"BOOSTED ODDS · 1 FOIL · 1 RARE+" });
    if (championshipCount > 0) vaultBuckets.push({ setId:vaultSet.id, type:"championship", count:championshipCount, label:"LEGACY SUPER PACK", subtitle:"BOOSTED ODDS · 1 FOIL · 1 RARE+" });
  }
  ...
  const vaultShelf = vaultBuckets.length ? `<section class="booster-vault-shelf ${vaultBuckets.length === 1 ? "single-pack" : ""}" aria-label="Openable booster packs">${vaultBuckets.map((bucket,index)=>{
    ...
    return `<button type="button" class="vault-pack-product pack-set-${bucket.setId}" data-open-vault-pack="${bucket.setId}:${bucket.type}" aria-label="Open ${info.displayName} ${bucket.label}">
      ...
      ${physicalBoosterPackMarkup({setId:bucket.setId,title:info.name,series:bucket.type==='standard'?'SERIES 1':'SUPER PACK',subtitle:bucket.subtitle,extraClass:`vault-product-pack ${bucket.type==='super'?'is-super-pack':''}`})}
      <strong>${info.displayName}</strong><em>${bucket.label}</em><small>TAP PACK TO OPEN</small>
    </button>`;
  }).join('')}</section>` : `<section class="booster-empty-stage"><div class="booster-empty-state"><span>VAULT EMPTY</span><h3>No unopened packs right now</h3><p>Win matches for boosters. Win full modes and tournaments for Super Packs.</p><button id="booster-empty-home" class="nav-button">Back to Main Menu</button></div></section>`;
...
function renderStore() {
  ...
  root.innerHTML=`<section class="store-screen premium-screen store-redesign" data-store-set="${rotation.setId}">
    ...
    <section class="store-set-banner">${setLogoMarkup(rotation.setId,"store-set-logo")}<div><span>FEATURED SET</span><strong>${setInfo?.displayName ?? setInfo?.name ?? rotation.setId}</strong><small>Rotates daily</small></div><div class="store-refresh-compact"><small>REFRESHES IN</small><b data-store-countdown>${formatStoreCountdown(rotation.msRemaining)}</b></div></section>
...
function renderSeasons() {
  ...
  const tierRoad = Array.from({length: SEASON_TIER_COUNT}, (_, i) => i + 1).map(tier => {
    ...
    const major = isFinalBossCard || tier % 10 === 0 || tier === 100;
    ...
    const rewardSub = isFinalBossCard ? `${reward.label} · FINAL BOSS` : reward.kind === "universe-points" ? `UNIVERSE POINTS` : setName;
  ...
  const roadmap = SEASON_1.roadmap.map((node,index)=>{const status=roadmapNodeStatus(node,new Date());return `<article class="season-release-chip ${status}"><span>${node.dateLabel}</span><b>${node.title}</b><small>${node.kicker}</small></article>`;}).join('');
  root.innerHTML = `<section class="seasons-screen premium-screen season-road-redesign">
    <section class="season-sticky-shell">
      <header class="season-road-hero"><div class="season-road-hero-copy"><span>WWE LEGACY · SEASON 1</span><h1><b>SEASON</b> <em>ONE</em></h1><p>THE ROAD TO THE FINAL BOSS</p><div class="season-compact-meta"><b>TIER ${progress.tier}</b><small>${claimable.length} REWARD${claimable.length===1?'':'S'} READY</small></div><div class="season-end-countdown"><small>SEASON ENDS IN</small><strong data-season-end-countdown>${seasonCountdown}</strong></div></div><div class="season-road-rock">${finalBossRockMarkup()}</div></header>
      ...
      ${message ? `<p class="setup-message">${message}</p>` : ''}
      <section class="season-free-pack-cta season-free-pack-strip ${free.available ? 'ready' : 'waiting'}"><button id="claim-free-pack" class="season-free-pack-button" ${free.available?'':'disabled'}><strong data-free-pack-action>${free.available?'CLAIM FREE BOOSTER':`NEXT FREE BOOSTER IN ${formatCountdown(free.msRemaining)}`}</strong></button></section>
    </section>
    <section class="season-road-toolbar"><div><span>100-TIER REWARD ROAD</span><small>Auto-focused on your current tier</small></div>${claimable.length?'<button id="claim-all-season" class="primary season-claim-all">CLAIM ALL READY</button>':''}</section>
    <section class="season-reward-road" aria-label="100 Tier Season Road"><span class="season-road-spine" aria-hidden="true"></span>${tierRoad}</section>
    <section class="season-release-road"><div class="season-road-toolbar"><div><span>SEASON 1 RELEASES</span><small>Live and upcoming sets</small></div><b>NEXT · ${next.title}</b></div><div class="season-release-strip">${roadmap}</div></section>
  </section>`;
  ...
  root.querySelectorAll('[data-claim-season-tier]').forEach(btn => btn.addEventListener('click', () => {
    try { const reward=claimSeasonTier(profile,Number(btn.dataset.claimSeasonTier));saveProfile(profile);message=reward.kind==='universe-points'?`Tier ${reward.tier} claimed: +${reward.amount} UP.`:reward.kind==='final-boss-card'?`Tier ${reward.tier} claimed: ${reward.name} unlocked.`:`Tier ${reward.tier} claimed: ${reward.amount} ${sets[reward.setId]?.name??reward.setId} booster${reward.amount===1?'':'s'}.`; }
  ...
  refreshSeasonClocks();
  const stickyShell = root.querySelector('.season-sticky-shell');
  if (seasonStickyScrollHandler) window.removeEventListener('scroll', seasonStickyScrollHandler);
  seasonStickyScrollHandler = () => stickyShell?.classList.toggle('is-compact', window.scrollY > 96);
  window.addEventListener('scroll', seasonStickyScrollHandler, { passive: true });
  seasonStickyScrollHandler();
  ...
  setTimeout(()=>{
    ...
    if(!target)return;
    const chromeHeight=document.querySelector('.legacy-gamebar')?.getBoundingClientRect().height??0;
    const rect=target.getBoundingClientRect();
    const top=Math.max(0,window.scrollY+rect.top-(chromeHeight+118));
    window.scrollTo({top,left:0,behavior:'auto'});
    requestAnimationFrame(seasonStickyScrollHandler);
  },120);
}
...
function renderChallenges() {
  ...
  const milestone = (setId, setName, m, type) => `<article class="milestone-row premium-milestone-row"><span class="milestone-icon">${uiIcon(type === 'foil' ? 'star' : 'trophy')}</span><div><b>${type === 'foil' ? 'FOIL' : 'COLLECTION'} ${m.percent}%</b><span>${m.reward} booster${m.reward===1?'':'s'}</span></div><button class="primary" data-claim-milestone="${setId}:${type}:${m.percent}">CLAIM</button></article>`;
  const milestoneSections = setRows.map(({set,progress,state,rewards}) => `<section class="challenge-section set-milestone-section ${setVisualClass(set.id)}"><div class="set-milestone-heading"><div class="set-milestone-logo">${setLogoMarkup(set.id,'challenge-set-logo')}</div><div><span>SET MILESTONES</span><h3>${set.displayName}</h3><small>${progress.ownedUnique}/${progress.total} UNIQUE · ${progress.foilUnique}/${progress.total} FOIL</small></div></div><div class="milestone-grid">${[...rewards.collection.map(m=>milestone(set.id,set.name,m,'collection')),...rewards.foil.map(m=>milestone(set.id,set.name,m,'foil'))].join('') || `<div class="milestone-progress-empty"><b>${Math.round((progress.ownedUnique/Math.max(1,progress.total))*100)}%</b><span>Next collection reward still in progress</span><i style="width:${Math.min(100,(progress.ownedUnique/Math.max(1,progress.total))*100)}%"></i></div>`}</div></section>`).join('');
...
function renderLiveEventHub() {
  ...
  const cards = towers.map((tower, index) => {
    ...
    const accent = tower.event.id === 'powerhouse-collision' ? 'powerhouse' : tower.event.id === 'submission-specialists' ? 'submission' : tower.event.id === 'legends-collide' ? 'legends' : tower.cadence === "daily" ? "daily" : tower.cadence === "three-day" ? "three-day" : tower.cadence === "birthday" ? "birthday" : "weekly";
    return `<article class="live-tower-hub-card compact-live-choice ${accent}" data-open-live-tower="${tower.key}">
      ...
      <div class="live-tower-hub-copy">
        ...
        <small class="live-choice-summary">${progress}/${LIVE_EVENT_LENGTH} MATCHES · SUPER PACK ON CLEAR</small>
        <div class="live-tower-hub-footer"><b class="live-tower-enter">${status === "COMPLETE" ? "VIEW COMPLETED TOWER" : status === "AVAILABLE" ? "ENTER TOWER" : `CONTINUE ${status}`} <i>›</i></b><div class="live-tower-timer"><small>ENDS IN</small><strong data-live-tower-expiry="${tower.nextAt.getTime()}">${formatCountdown(tower.msRemaining)}</strong></div></div>
      </div>
    </article>`;
  }).join("");
  ...
  const mitbCard = `<article class="live-tower-hub-card compact-live-choice mitb money-in-bank-live-card" id="open-money-in-bank" role="button" tabindex="0">
    <span class="legacy-mode-beams" aria-hidden="true"></span><div class="live-tower-hub-copy"><span class="live-tower-cadence">DAILY · THREE LIVES · ${mitbStatus}</span><h3 class="live-event-split-title title-mitb"><span>MONEY IN THE</span><b>BANK</b></h3><small class="live-choice-summary">${mitbProgress}/${LADDER_LENGTH} LEVELS · ${mitbActive?mitbRun.lives:LADDER_LIVES} LIVES · SUPER PACK ON CLEAR</small><div class="live-tower-hub-footer"><b class="live-tower-enter">${mitb.dailyCleared?'VIEW COMPLETED RUN':mitbActive?`CONTINUE ${mitbStatus}`:'ENTER MONEY IN THE BANK'} <i>›</i></b><div class="live-tower-timer"><small>RESETS</small><strong>LOCAL MIDNIGHT</strong></div></div></div><div class="mitb-briefcase" aria-hidden="true">$</div>
  </article>`;
  root.innerHTML = `<section class="live-events-hub premium-screen">
    <header class="live-events-hub-heading">${modeLogoMarkup("live-event", false)}<p>Money in the Bank plus limited-time towers rotating every day, every three days and every week.</p></header>
...
function renderLiveEventTowerDetail(towerKey) {
  ...
  const normalVictoryPackTarget = Math.max(0, LIVE_EVENT_LENGTH - 1);
  const earnedPacks = Math.min(run?.stage ?? 0, normalVictoryPackTarget);
  ...
  const opponentIds = active ? run.opponents.filter(id => superstarById[id]) : [...previewThemed, ...previewFallback].slice(0, LIVE_EVENT_LENGTH);
  ...
  root.innerHTML = `<section class="live-tower-detail cadence-${tower.cadence} event-${event.id}">
    ...
    <section class="live-tower-detail-hero compact-live-detail-hero">
      ...
      <div class="live-tower-detail-stats compact-live-detail-stats"><article><span>TIME LEFT</span><b data-live-tower-expiry="${tower.nextAt.getTime()}">${formatCountdown(tower.msRemaining)}</b></article><article><span>MATCHES</span><b>${Math.min(run?.stage ?? 0,LIVE_EVENT_LENGTH)}/${LIVE_EVENT_LENGTH}</b></article><article><span>VICTORY PACKS</span><b>${earnedPacks}/${normalVictoryPackTarget}</b></article></div>
    </section>
    ${message ? `<div class="live-event-message-banner"><span>${message}</span></div>` : ''}
    <section class="live-tower-detail-body">${command}<section class="live-tower-route"><div class="live-tower-section-head"><span>EVENT ROUTE</span><strong>${active ? `Current Opponent · ${focusOpponent?.name ?? 'Opponent'}` : `${LIVE_EVENT_LENGTH} opponents · Super Pack on clear`}</strong></div><div class="live-tower-route-strip">${route}</div></section></section>
  </section>`;
...
function renderLadder() {
  ...
  const ladderRows = (run?.opponents ?? ladder.dailyOpponents ?? []).map((id,index) => {
    const star = superstarById[id];
    const focusIndex = active ? run.rung : 0;
    const state = run ? (index < run.rung ? "cleared" : index === focusIndex ? "current" : "upcoming") : (index === 0 ? "current" : "upcoming");
    const stateLabel = state === "cleared" ? "Defeated" : state === "current" ? (active ? "Next" : "Level 1") : "Waiting";
    return `<article class="ladder-rung mode-run-node redesigned-ladder-node mitb-opponent-node ${state}" data-mitb-opponent-index="${index}"><span class="mode-run-index">${index+1}</span><div class="mitb-opponent-card-wrap">${superstarPreviewCardMarkup(id,"mitb-opponent-card")}</div><div class="mode-run-node-copy"><b>${star?.name ?? id}</b><small>Level ${index+1}</small><em>${stateLabel}</em></div></article>`;
  }).join("");
  const statusText = ladder.dailyCleared ? "Today's Money in the Bank run is complete. A new 8-Superstar field arrives at local midnight." : run?.status === "failed" ? "All three lives are gone. Restart today's Money in the Bank run from Level 1." : active ? `Level ${run.rung+1} of ${LADDER_LENGTH} · ${superstarById[currentLadderOpponent(profile)]?.name}` : "Eight random Superstars · three lives · one daily Super Pack.";
  ...
  const commandMarkup = ladder.dailyCleared
    ? `<section class="mode-run-command ladder-run-command redesigned-ladder-command complete"><div class="mode-run-command-copy"><span>DAILY CHALLENGE COMPLETE</span><strong>Money in the Bank Cleared</strong><small>Return tomorrow for eight new random opponents.</small></div><button id="ladder-back-live-events" class="nav-button">Back to Live Events</button></section>`
    : active
      ? `<section class="mode-run-command ladder-run-command redesigned-ladder-command"><div class="mode-run-command-copy"><span>NEXT LEVEL · ${run.rung+1}/${LADDER_LENGTH}</span><strong>${superstarById[currentLadderOpponent(profile)]?.name}</strong><small>${superstarById[run.superstarId].name} · ${'●'.repeat(lives)}${'○'.repeat(LADDER_LIVES-lives)} lives</small></div><button id="continue-ladder" class="start-match mode-run-primary">Fight Level ${run.rung+1}</button></section>`
      : `<section class="ladder-picker horizontal-selector mode-run-picker"><div class="mode-run-picker-head"><div><span>YOUR SUPERSTAR</span><h3>${chosenStar?.name ?? 'Choose your Superstar'}</h3></div><small>Today's opponents lock on your first run</small></div>${selectionCarouselMarkup(unlocked,chosenId,'ladder-select')}<button id="start-ladder" class="start-match mode-run-primary">${run?.status === 'failed' ? 'Restart at Level 1' : 'Start Money in the Bank'}</button></section>`;
  root.innerHTML = `<section class="ladder-screen premium-screen compact-mode-run premium-run-screen redesigned-ladder-screen mitb-run-screen ${active?'is-active-run':'is-run-setup'}"><section class="feature-hero ladder-feature single-feature-hero mode-run-hero mitb-run-hero"><div class="feature-shade"></div><div class="feature-copy mode-run-hero-copy">${modeLogoMarkup("ladder",true)}<p>Eight random opponents · three lives · resets every day.</p></div><div class="ladder-summary mode-run-summary"><div><b>${ladder.clears ?? 0}</b><span>Career Clears</span></div><div><b>${ladder.bestRung ?? 0}/${LADDER_LENGTH}</b><span>Best</span></div><div><b>${'●'.repeat(lives)}${'○'.repeat(LADDER_LIVES-lives)}</b><span>Lives</span></div><div><b>${ladder.dailyCleared ? 'DONE' : 'LIVE'}</b><span>Today</span></div></div></section>${statusMarkup}<section class="ladder-bottom-shell">${commandMarkup}${ladderRows ? `<section class="ladder-stack mode-run-progress ladder-progress-grid redesigned-ladder-progress mitb-opponent-carousel" aria-label="Today's Money in the Bank opponents">${ladderRows}</section>` : ''}</section>${run?.status==='failed' ? `<button id="new-ladder" class="start-match mode-run-primary">Restart Money in the Bank</button>` : ''}</section>`;
  ...
  requestAnimationFrame(()=>{
    const carousel=root.querySelector('.mitb-opponent-carousel');
    const focusIndex=active ? Math.max(0,Math.min(LADDER_LENGTH-1,run.rung)) : 0;
    const focus=carousel?.querySelector(`[data-mitb-opponent-index="${focusIndex}"]`);
    if(!carousel||!focus)return;
    const left=Math.max(0,focus.offsetLeft-(carousel.clientWidth-focus.offsetWidth)/2);
...
function renderKingOfTheRing() {
  ...
  if (run?.status === "cleared" && !run.coronationSeen && !run.rewardClaimedSetId) {
  ...
  if (run?.status === "cleared" && run.coronationSeen && !run.rewardClaimedSetId) {
    const choices = Array.isArray(run.rewardChoices) ? run.rewardChoices : prepareKingOfTheRingReward(profile, playerReleasedCollectibleSetIds(new Date()), Math.random);
    saveProfile(profile);
    const choiceMarkup = choices.map(setId => `<button type="button" class="kotr-reward-choice" data-kotr-reward-set="${setId}">${physicalBoosterPackMarkup({setId,title:sets[setId]?.name ?? "WWE LEGACY",series:"SUPER PACK",subtitle:"BOOSTED ODDS · 1 RARE+",extraClass:"kotr-choice-pack"})}<strong>${sets[setId]?.displayName ?? sets[setId]?.name ?? setId}</strong><small>CHOOSE THIS SUPER PACK</small></button>`).join('');
    root.innerHTML = `<section class="kotr-reward-screen premium-screen"><header class="premium-hub-heading hub-kotr"><span class="premium-hub-kicker">CROWN CLAIMED</span><h1><span>CHOOSE YOUR</span> <b>REWARD</b></h1><p>Choose one boosted-odds Super Pack. The other two return to the vault.</p></header><section class="kotr-reward-king"><span>REIGNING KING</span><strong>${name(run.superstarId)}</strong><small>${state.clears} career crown${state.clears===1?'':'s'}</small></section><div class="kotr-reward-grid">${choiceMarkup}</div></section>`;
    root.querySelectorAll('[data-kotr-reward-set]').forEach(btn=>btn.addEventListener('click',()=>{
      try {
        const setId = claimKingOfTheRingReward(profile, btn.dataset.kotrRewardSet);
        saveProfile(profile);
        message = `${sets[setId]?.displayName ?? sets[setId]?.name ?? setId} Super Pack claimed. ${name(run.superstarId)} remains the reigning King of the Ring.`;
        renderKingOfTheRing();
      } catch (error) { message = error.message; renderKingOfTheRing(); }
    }));
    return;
    ...
    <div class="clean-splash-content">
      ...
      <section class="season-one-ad" aria-label="Season 1 Final Boss promotion">
        ...
        <div class="season-ad-copy">
          ...
          <p>Climb the 100-tier Season Road to assemble <b>The Final Boss</b>. Exclusive moves, his Action, Finisher and Entrance unlock along the way. Tier 100 unlocks the Foil The Rock — Final Boss Superstar card.</p>
  ...
  root.innerHTML = `<section class="main-menu-screen premium-screen legacy-home-v3">
    ...
    <button id="menu-season-overview" class="legacy-season-event ${attentionClass("seasons")}" aria-label="Open Season 1 hub">${attentionBadge("seasons")}
      ...
      <span class="legacy-season-copy">
        <em><i></i> THE ROAD TO THE FINAL BOSS</em>
        <strong class="season-home-title"><span>SEASON</span> <b>ONE</b></strong>
    ...
    <section class="legacy-command-rack legacy-home-destinations" aria-label="WWE Legacy destinations">
      ...
      <button id="menu-owned-collection" class="legacy-command-tile command-collection"><span class="legacy-command-photo">${menuSuperstarPhotoMarkup("liv-morgan","Liv Morgan")}</span><span class="legacy-command-glow"></span><span class="legacy-command-copy"><em>${ownedCopies} OWNED COPIES</em>${homeHubSplitTitle("MY", "COLLECTION")}<small>Owned cards · favourites · foils</small><b>OPEN <i>›</i></b></span></button>
...
function layeredFrontOverlayMarkup(card) {
  ...
  if (card.kind === "move") {
    ...
    return `<span class="ccg-live-front-data ccg-live-front-move" aria-hidden="true">
      ...
      <strong class="ccg-live-name ${nameClass}">${card.name}</strong>
...
function renderCollection() {
  ...
  root.innerHTML = `<section class="collection-screen premium-screen ${themeClass} collection-density-screen collection-compact-redesign">
    ${collectionView === "owned" ? premiumHubHeading("MY", "COLLECTION", "OWNED CARDS", "Your cards · favourites · Foils", "hub-collection") : premiumHubHeading("CARD", "CATALOGUE", "ALL ACTIVE CARDS", "Search · filter · inspect", "hub-catalogue")}
    <section class="collection-quickbar"><div class="collection-view-switch"><button id="collection-owned-view" class="nav-button ${collectionView === 'owned' ? 'active' : ''}">My Collection</button><button id="collection-catalogue-view" class="nav-button ${collectionView === 'catalogue' ? 'active' : ''}">Card Catalogue</button></div><div class="collection-quick-stats"><span><b>${collectionView === 'owned' ? ownedUniqueHere : baseCards.length}</b><small>${collectionView === 'owned' ? 'OWNED HERE' : 'IN VIEW'}</small></span><span><b>${unlocked}/${starCards.length}</b><small>SUPERSTARS</small></span><span><b>${ownedUniqueAll}</b><small>UNIQUE OWNED</small></span><span><b>${visibleCards.length}</b><small>CATALOGUE</small></span></div></section>
...
function renderCardCatalogue() {
  ...
  root.innerHTML = `<section class="catalogue-screen premium-screen theme-catalogue-all catalogue-density-screen catalogue-compact-redesign">
    ...
    <section class="catalogue-quickbar"><div class="catalogue-master-stats"><span><b>${visibleCards.length}</b><small>RELEASED</small></span><span><b>${ownedUnique}</b><small>OWNED</small></span><span><b>${filtered.length}</b><small>MATCHING</small></span></div><button id="catalogue-my-collection" class="nav-button catalogue-owned-link">MY COLLECTION</button></section>
...
function renderDeckBuilder() {
  ...
  root.innerHTML = `<section class="deck-builder-screen deck-lab-screen premium-screen">
    ...
    <section class="deck-validity-panel"><div class="section-title"><h3>Deck Validity</h3><span class="${health.healthy ? 'valid' : 'invalid'}">${health.healthy ? 'VALID' : 'CHECK REQUIRED'}</span></div>${violations}<div class="deck-builder-actions"><button id="optimize-deck" class="nav-button">Optimize Owned</button><button id="reset-recommended" class="nav-button">Build Toward Recommended</button><button id="save-deck" class="start-match" ${health.healthy ? '' : 'disabled'}>Save Deck</button></div></section>
  </section>${renderDeckLabInspectOverlay()}`;
  ...
  $("#optimize-deck")?.addEventListener("click", () => { deckDraft = optimizeDeck(profile, deckBuilderStarId); message = deckDraft.length === 60 ? "Deck rebuilt from your Collection, prioritising the recommended build and legal replacements." : `Every legal owned option was used. ${60-deckDraft.length} slots remain.`; renderDeckBuilder(); });
  $("#reset-recommended")?.addEventListener("click", () => { deckDraft = buildBestOwnedRecommendedDraft(profile, deckBuilderStarId); deckLabEntranceId = recommendedEntranceId(profile, deckBuilderStarId); message = `Best owned version built: ${60-recommendedDeckMissingCount(deckBuilderStarId,deckDraft)}/60 recommended slots matched.`; renderDeckBuilder(); });
...
function playCard(playerId, index) {
  ...
  try {
    ...
    message = game.state().pendingActionDiscard?.playerId === playerId ? `${card.name}: choose ${game.state().pendingActionDiscard.count} page to ditch.` : `${nameFor(playerId)} played ${card.name}.`;
...
function renderCommandBar() {
  ...
  let prompt = state.phase === "MATCH_OVER" ? (state.winner ? `${nameFor(state.winner)} wins by ${state.finish.type.toUpperCase()}!` : `Match ends by ${state.finish.type.toUpperCase()}!`) : owner === CPU ? `${nameFor(CPU)} is thinking…` : actionDiscard ? `${cardNameFor(actionDiscard.cardId)} — choose ${actionDiscard.count} page${actionDiscard.count===1?"":"s"} to ditch.` : `Your turn — choose a page or action.`;
  const autoEligibility = state.phase === "COUNTER" && owner === HUMAN ? autoCounterEligibility(state, HUMAN, state.proposedMove?.card) : null;
  if (state.phase === "TRIGGER_RESPONSE" && owner === HUMAN && triggered) prompt = `${triggeredCard?.name ?? "Triggered Action"} is available ${triggered.reason ?? "now"} — use it or decline.`;
  ...
  return `<section class="match-command ${state.phase === "MATCH_OVER" ? "match-over" : ""}">
    ...
    <div class="command-actions">
      ${owner === HUMAN && state.phase === "TRIGGER_RESPONSE" && triggered ? `<button id="use-triggered-special" class="primary show-command-button">USE ${triggeredCard?.name?.toUpperCase?.() ?? "ACTION"}</button><button id="decline-triggered-special" class="secondary show-command-button">DECLINE</button>` : ""}
...
function victoryBoosterSetId(state) {
  if (activeMode === "live-event") {
    const entry = activeLiveEventTowerKey ? liveEventTowerState(profile, activeLiveEventTowerKey, new Date()) : null;
    return entry?.state?.activeRun?.rewardSetId ?? entry?.tower?.event?.rewardSetId ?? "summerslam-series-1";
  }
  if (activeMode === "ladder") return ladderState(profile).activeRun?.setId ?? "summerslam-series-1";
  if (activeMode === "championship") {
    const run = championshipRoadState(profile).activeRun;
    return championshipRoadSectionForStage(run?.stage ?? 0)?.setId ?? "summerslam-series-1";
  }
  const starSet = state?.players?.[HUMAN]?.superstar?.setId;
  return isPlayerReleasedSetId(starSet) ? starSet : "summerslam-series-1";
}
...
function handleCompletedMatch() {
  ...
  const victorySetId = victoryBoosterSetId(state);
  const awardNormalVictoryPack = (modeCompleted = false) => {
    if (modeCompleted) return "";
    const awarded = grantVictoryBooster(profile, result, victorySetId);
    return awarded ? ` +1 ${sets[victorySetId]?.displayName ?? victorySetId} victory booster.` : "";
  };

  if (activeMode === "king-of-the-ring") {
    const outcome = recordKingOfTheRingMatch(profile, result);
    const run = kingOfTheRingState(profile).activeRun;
    if (outcome.status === "advance") message = `${KING_OF_THE_RING_ROUNDS[run.stage - 1]} won! One step closer to the crown. Next: ${KING_OF_THE_RING_ROUNDS[run.stage]}.`;
      ...
      prepareKingOfTheRingReward(profile, playerReleasedCollectibleSetIds(new Date()), Math.random);
      message = `KING OF THE RING! ${state.players[HUMAN].superstar.name} won three straight matches. The coronation and choose-one Super Pack reward await.`;
    }
    message += awardNormalVictoryPack(outcome.status === "cleared") + seasonXpText;
  ...
  if (activeMode === "championship") {
    ...
    const run = championshipRoadState(profile).activeRun;
    const checkpointSetId = outcome.sectionCleared?.setId ?? victorySetId;
    if (outcome.status === "advance") {
      const checkpointText = outcome.sectionCleared ? ` ${outcome.sectionCleared.label} complete.` : '';
      message = `${CHAMPIONSHIP_STAGES[run.stage - 1]} won!${checkpointText} Return to Championship Road for ${CHAMPIONSHIP_STAGES[run.stage]}.`;
    }
      ...
      message = `CHAMPIONSHIP ROAD CLEARED on ${CHAMPIONSHIP_DIFFICULTIES[run.difficultyId]?.label ?? run.difficultyId}! Final region complete and a boosted-odds Super Pack has been awarded.${outcome.unlockedDifficulty ? ` ${CHAMPIONSHIP_DIFFICULTIES[outcome.unlockedDifficulty].label} is now unlocked.` : ''}`;
    }
    message += awardNormalVictoryPack(outcome.status === "cleared") + seasonXpText;
    refreshCareerAchievements(profile);
    saveProfile(profile);
  ...
  if (activeMode === "live-event") {
    ...
    if (outcome.status === "advance") {
      ...
      message = `${tower.cadenceLabel} match cleared! Next: ${nextStage.label} vs ${nextOpponent}.`;
      ...
      message = `${tower.cadenceLabel} CLEARED! A boosted-odds ${sets[rewardSetId]?.displayName ?? rewardSetId} Super Pack has been awarded.`;
    }
    message += awardNormalVictoryPack(outcome.status === "cleared") + seasonXpText;
    refreshCareerAchievements(profile);
    saveProfile(profile);
  ...
  if (activeMode !== "ladder") {
    message += awardNormalVictoryPack(false) + seasonXpText;
    refreshCareerAchievements(profile);
    saveProfile(profile);
    return;
  }

  const outcome = recordLadderMatch(profile, result, new Date());
  const run = ladderState(profile).activeRun;
  if (outcome.status === "advance") message = `Level cleared! ${run.lives} lives remain. Return to Money in the Bank for Level ${run.rung + 1}.`;
  ...
  else if (outcome.status === "cleared") { message = `MONEY IN THE BANK CLEARED! Today’s 8-Superstar tower is complete and a boosted-odds Super Pack has been awarded.`; }
  message += awardNormalVictoryPack(outcome.status === "cleared") + seasonXpText;
  refreshCareerAchievements(profile);
  saveProfile(profile);
}
...
function renderMatchResults() {
  ...
  const isLadder = activeMode === "ladder";
  const isChampionship = activeMode === "championship";
  ...
  const ladderRun = isLadder ? ladderState(profile,new Date()).activeRun : null;
  const champRun = isChampionship ? championshipRoadState(profile).activeRun : null;
  const championshipRewardStage = Math.max(0, (champRun?.stage ?? 1) - (humanWon ? 1 : 0));
  const rewardSetId = isLiveEvent
    ? (liveRun?.rewardSetId ?? liveTower?.event.rewardSetId ?? "summerslam-series-1")
    : isLadder
      ? (ladderRun?.setId ?? "summerslam-series-1")
      : isChampionship
        ? (championshipRoadSectionForStage(championshipRewardStage)?.setId ?? "summerslam-series-1")
        : (isPlayerReleasedSetId(humanStar.setId) ? humanStar.setId : "summerslam-series-1");
  ...
  const championshipCleared = isChampionship && humanWon && champRun?.status === "cleared";
  const ladderCleared = isLadder && humanWon && ladderRun?.status === "cleared";
  let rewardLine = humanWon ? `1 × ${rewardName} Victory Booster` : "NO REWARD";
  let rewardMarkup = humanWon
    ? `<div class="results-pack-reward">${physicalBoosterPackMarkup({setId:rewardSetId,title:sets[rewardSetId]?.name ?? "WWE LEGACY",series:"SERIES 1",subtitle:"VICTORY BOOSTER",extraClass:"results-booster"})}<strong>${rewardLine}</strong></div>`
    : `<strong>${rewardLine}</strong>`;
  let showRewardButton = humanWon;

  if (isKingOfTheRing) {
    const kotrRun = kingOfTheRingState(profile).activeRun;
    const tournamentWon = humanWon && kotrRun?.status === "cleared";
    if (tournamentWon) {
      rewardLine = `Super Pack choice after coronation`;
      rewardMarkup = `<div class="results-crown-reward"><span>♛</span><strong>${rewardLine}</strong><small>THE KING OF THE RING CORONATION IS NEXT</small></div>`;
      showRewardButton = false;
    } else if (humanWon) {
      rewardLine = `1 × ${rewardName} Victory Booster · ${KING_OF_THE_RING_ROUNDS[Math.max(0,(kotrRun?.stage ?? 1)-1)]} cleared`;
      rewardMarkup = `<strong>${rewardLine}</strong>`;
      showRewardButton = true;
    } else {
      rewardLine = "NO REWARD · Eliminated";
      rewardMarkup = `<strong>${rewardLine}</strong>`;
      showRewardButton = false;
    }
  } else if (isLadder) {
    if (ladderCleared) {
      rewardLine = `1 × Super Pack`;
      rewardMarkup = `<div class="results-pack-reward">${physicalBoosterPackMarkup({setId:rewardSetId,title:"SUPER PACK",series:"MONEY IN THE BANK",subtitle:"BOOSTED ODDS · 1 RARE+",extraClass:"results-booster is-super-pack"})}<strong>${rewardLine}</strong></div>`;
    } else if (humanWon) {
      rewardLine = `1 × ${rewardName} Victory Booster · Level ${ladderRun?.rung ?? 0}/${LADDER_LENGTH} cleared`;
      rewardMarkup = `<strong>${rewardLine}</strong>`;
    } else {
      rewardLine = `NO REWARD · ${ladderRun?.lives ?? 0} lives remaining`;
      rewardMarkup = `<strong>${rewardLine}</strong>`;
    }
    showRewardButton = humanWon;
  } else if (isChampionship) {
    if (championshipCleared) {
      rewardLine = `1 × Super Pack`;
      rewardMarkup = `<div class="results-pack-reward">${physicalBoosterPackMarkup({setId:rewardSetId,title:"SUPER PACK",series:"CHAMPIONSHIP ROAD",subtitle:"BOOSTED ODDS · 1 RARE+",extraClass:"results-booster is-super-pack"})}<strong>${rewardLine}</strong></div>`;
    } else if (humanWon) {
      rewardLine = `1 × ${rewardName} Victory Booster · Match ${champRun?.stage ?? 0}/${CHAMPIONSHIP_ROAD_LENGTH} cleared`;
      rewardMarkup = `<strong>${rewardLine}</strong>`;
    } else {
      rewardLine = "NO REWARD · Retry the current Championship Road match";
      rewardMarkup = `<strong>${rewardLine}</strong>`;
    }
    showRewardButton = humanWon;
  } else if (isLiveEvent) {
    if (humanWon && liveFinalClear) {
      rewardLine = `1 × Super Pack`;
      rewardMarkup = `<div class="results-pack-reward">${physicalBoosterPackMarkup({setId:rewardSetId,title:"SUPER PACK",series:liveTower?.cadenceLabel ?? "LIVE EVENT",subtitle:"BOOSTED ODDS · 1 RARE+",extraClass:"results-booster is-super-pack"})}<strong>${rewardLine}</strong></div>`;
      showRewardButton = true;
    } else if (humanWon) {
      rewardLine = `1 × ${rewardName} Victory Booster · Match ${Math.min(liveRun?.stage ?? 0, LIVE_EVENT_LENGTH)}/${LIVE_EVENT_LENGTH} cleared`;
      rewardMarkup = `<strong>${rewardLine}</strong>`;
      showRewardButton = true;
    } else {
      rewardLine = `NO REWARD · Retry Match ${Math.min((liveRun?.stage ?? 0) + 1, LIVE_EVENT_LENGTH)}`;
      rewardMarkup = `<strong>${rewardLine}</strong>`;
      showRewardButton = false;
    }
  }
