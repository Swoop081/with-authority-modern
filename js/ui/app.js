import { assetUrl, BUILD_VERSION } from "../config/build.js?v=0.13.26";
import { superstars } from "../data/superstars.js?v=0.13.26";
import { decks } from "../data/decks.js?v=0.13.26";
import { sets } from "../data/sets.js?v=0.13.26";
import { playerReleasedCollectibleSetIds, isPlayerReleasedSetId, isPlayerVisibleSuperstar } from "../data/release.js?v=0.13.26";
import { collectionCards, setCollection, setCollections, cardsForSet } from "../data/collection.js?v=0.13.26";
import { artworkFor, superstarArtwork, menuSuperstarPhotoFor, finalBossRockMenuArtwork, superstarCardArtFor, superstarHeadshotFor, finishedCardArtFor, legacyFinishedCardArtFor, layeredCardArtFor } from "../data/artwork.js?v=0.13.26";
import { STARTER_CHOICES, createProfile, hasSuperstar, loadProfile, saveProfile, resetProfile, setDeckAssistance, ownedCount, addUniversePoints } from "../data/profile.js?v=0.13.26";
import { openBooster, openLadderCompletionPack, openChampionshipPack, grantBooster, boosterCreditsFor, finalizePackUniversePoints } from "../data/boosters.js?v=0.13.26";
import { STORE_BOOSTER_PRICE, STORE_SUPERSTAR_PRICE, storeRotation, storeSuperstars, storeLeadOffCards, purchaseStoreBooster, purchaseStoreSuperstar } from "../data/store.js?v=0.13.26";
import { randomExhibitionOpponent } from "../data/matchmaking.js?v=0.13.26";
import { buildPlayableDeck, findPackUpgrades, applyUpgrade } from "../data/deck-assistant.js?v=0.13.26";
import { MatchEngine } from "../engine/MatchEngine.js?v=0.13.26";
import { canPlayMomentum, canPlayEntrance, canPlayAction, canPlaySupport, canPlayManager, canPlaySpecial, effectiveTotalMomentum, moveEligibility, canCounter, counterEligibility, autoCounterEligibility, autoCounterCost, canAttemptPin, canPlayPinEscape, submissionThreshold, canReturnToRing, canFollowOutside } from "../engine/rules.js?v=0.13.26";
import { totalMomentum } from "../engine/utils.js?v=0.13.26";
import { healthZone } from "../engine/health.js?v=0.13.26";
import { decisionOwner } from "../ai/WrestlingAI.js?v=0.13.26";
import { advanceCpuUntilHuman } from "./turn-driver.js?v=0.13.26";
import { LADDER_LIVES, LADDER_LENGTH, ladderState, startLadderRun, currentLadderOpponent, recordLadderMatch } from "../data/ladder.js?v=0.13.26";
import { KING_OF_THE_RING_ROUNDS, kingOfTheRingState, startKingOfTheRing, currentKingOfTheRingOpponent, recordKingOfTheRingMatch, prepareKingOfTheRingReward, markKingOfTheRingCoronationSeen, claimKingOfTheRingReward, resetKingOfTheRing } from "../data/king-of-the-ring.js?v=0.13.26";
import { CHAMPIONSHIP_ROAD_LENGTH, CHAMPIONSHIP_STAGES, CHAMPIONSHIP_DIFFICULTY_ORDER, CHAMPIONSHIP_DIFFICULTIES, CHAMPIONSHIP_ROAD_SECTIONS, CHAMPIONSHIP_ROAD_OPPONENTS, championshipRoadState, championshipDifficultyUnlocked, championshipRoadDifficultyModifier, championshipRoadSectionForStage, startChampionshipRoad, currentChampionshipOpponent, recordChampionshipMatch, resetChampionshipRoad } from "../data/championship-road.js?v=0.13.26";
import { LIVE_EVENT_LENGTH, LIVE_EVENT_WIN_UP, LIVE_EVENT_CLEAR_BOOSTERS, activeLiveEventTowers, liveEventTowerByKey, liveEventTowerState, startLiveEventTower, currentLiveEventTowerOpponent, currentLiveEventTowerStage, recordLiveEventTowerMatch, liveEventRotation, liveEventStage, weeklyLiveEventState } from "../data/live-events.js?v=0.13.26";
import { challengeState, claimChallenge, recordCompletedMatchChallenges } from "../data/challenges.js?v=0.13.26";
import { CAREER_MODES, careerRecord, achievementProgress, recordCareerMatch, refreshCareerAchievements } from "../data/career.js?v=0.13.26";
import { setProgressState, collectionProgress, availableMilestoneRewards, claimMilestone } from "../data/set-progression.js?v=0.13.26";
import { MOVE_TYPE_LABELS } from "../data/move-types.js?v=0.13.26";
import { COUNTER_STATE_LABELS, SUBMISSION_TARGET_LABELS } from "../data/counter-states.js?v=0.13.26";
import { CATALOGUE_PAGE_SIZE, defaultCatalogueFilters, catalogueOptions, filterAndSortCatalogue, superstarIdsForCard, isSharedCard } from "../data/catalogue.js?v=0.13.26";
import { DECK_LAB_CATEGORIES, createDeckDraft, recommendedDeckDraft, optimizeDeck, aggregateDeck, eligibleOwnedCards, allOwnedEntrances, ownedCardsForCategory, addCardToDraft, removeCardFromDraft, replaceLeadOffSlot, validateDeckDraft, materializeDraft, leadOffIds, buildOwnedRecommendedDraft, buildBestOwnedRecommendedDraft, recommendedDeckComparison, recommendedEntranceId, recommendedDeckMissingCount, autoFillOwnedDraft, recommendedCategoryCounts, currentCategoryCounts, cardEligibilityForSuperstar, entranceEligibilityForSuperstar, selectedEntranceId, setSelectedEntrance, ownedTotal, categoryForCard } from "../data/deck-builder.js?v=0.13.26";
import { RECOMMENDED_DECK_SHAPE } from "../data/deck-health.js?v=0.13.26";
import { SEASON_1, SEASON_TIER_COUNT, XP_PER_TIER, MATCH_XP, seasonState, seasonTier, seasonLevelProgress, seasonTimeRemaining, nextRoadmapNode, roadmapNodeStatus, awardMatchSeasonXp, tierReward, claimSeasonTier, claimAllSeasonTiers, freePackStatus, claimFreeSeasonBooster } from "../data/seasons.js?v=0.13.26";
import { GAME_RULE_SECTIONS, PIN_CHANCE_TABLE, LIVE_EVENT_WEEK } from "../data/game-rules.js?v=0.13.26";
import { SAVE_FILENAME, exportSaveToFiles, readSaveFile, saveImportRollback, loadImportRollback, clearImportRollback, backupMetadata } from "../data/save-backup.js?v=0.13.26";
...
function renderBoosters() {
  ...
  const packTitle=currentPackType==="ladder"?"MONEY IN THE BANK":currentPackType==="championship"?"CHAMPIONSHIP ROAD":setInfo.name.toUpperCase();
  ...
  for (const vaultSet of vaultSets) {
    ...
    if (ladderCount > 0) vaultBuckets.push({ setId:vaultSet.id, type:"ladder", count:ladderCount, label:"MONEY IN THE BANK", subtitle:"1 FOIL · 1 VERY RARE+" });
...
function renderSeasons() {
  ...
  const tierRoad = Array.from({length: SEASON_TIER_COUNT}, (_, i) => i + 1).map(tier => {
    ...
    return `<article id="season-tier-${tier}" class="season-road-node ${tier%2?'road-left':'road-right'} ${reached?'reached':''} ${claimed?'claimed':''} ${current?'current':''} ${major?'major':''} ${isFinalBossCard?'final-boss-node':''}">
...
function renderLadder() {
  ...
  const ladderRows = (run?.opponents ?? ladder.dailyOpponents ?? []).map((id,index) => {
    const star = superstarById[id], state = run ? (index < run.rung ? "cleared" : index === run.rung && active ? "current" : "upcoming") : "upcoming";
    const stateLabel = state === "cleared" ? "Defeated" : state === "current" ? "Next" : "Waiting";
    return `<div class="ladder-rung mode-run-node redesigned-ladder-node ${state}"><span class="mode-run-index">${index+1}</span><div class="ladder-portrait mode-run-node-portrait">${portraitMarkup(id,star?.name ?? id)}</div><div class="mode-run-node-copy"><b>${star?.name ?? id}</b><small>Level ${index+1}</small><em>${stateLabel}</em></div></div>`;
  }).join("");
  const statusText = ladder.dailyCleared ? "Today's Money in the Bank run is complete. A new 8-Superstar field arrives at local midnight." : run?.status === "failed" ? "All three lives are gone. Restart today's Money in the Bank run from Level 1." : active ? `Level ${run.rung+1} of ${LADDER_LENGTH} · ${superstarById[currentLadderOpponent(profile)]?.name}` : "Eight random Superstars · three lives · one daily completion reward.";
  ...
  root.innerHTML = `<section class="ladder-screen premium-screen compact-mode-run premium-run-screen redesigned-ladder-screen ${active?'is-active-run':'is-run-setup'}"><section class="feature-hero ladder-feature single-feature-hero mode-run-hero">${modePortraits([chosenId],"feature-art")}<div class="feature-shade"></div><div class="feature-copy mode-run-hero-copy">${modeLogoMarkup("ladder",true)}<p>Eight random opponents · three lives · resets every day.</p></div><div class="ladder-summary mode-run-summary"><div><b>${ladder.clears ?? 0}</b><span>Career Clears</span></div><div><b>${ladder.bestRung ?? 0}/${LADDER_LENGTH}</b><span>Best</span></div><div><b>${'●'.repeat(lives)}${'○'.repeat(LADDER_LIVES-lives)}</b><span>Lives</span></div><div><b>${ladder.dailyCleared ? 'DONE' : 'LIVE'}</b><span>Today</span></div></div></section>${statusMarkup}<section class="ladder-bottom-shell">${commandMarkup}${ladderRows ? `<section class="ladder-stack mode-run-progress ladder-progress-grid redesigned-ladder-progress" aria-label="Today's Ladder progress">${ladderRows}</section>` : ''}</section>${run?.status==='failed' ? `<button id="new-ladder" class="start-match mode-run-primary">Restart Money in the Bank</button>` : ''}</section>`;
...
function renderKingOfTheRing() {
  ...
  if (run?.status === "cleared" && run.coronationSeen && !run.rewardClaimedSetId) {
    ...
    saveProfile(profile);
    const choiceMarkup = choices.map(setId => `<button type="button" class="kotr-reward-choice" data-kotr-reward-set="${setId}">${physicalBoosterPackMarkup({setId,title:sets[setId]?.name ?? "WWE LEGACY",series:"SERIES 1",subtitle:"KING OF THE RING REWARD",extraClass:"kotr-choice-pack"})}<strong>${sets[setId]?.displayName ?? sets[setId]?.name ?? setId}</strong><small>CHOOSE THIS BOOSTER</small></button>`).join('');
    root.innerHTML = `<section class="kotr-reward-screen premium-screen"><header class="premium-hub-heading hub-kotr"><span class="premium-hub-kicker">CROWN CLAIMED</span><h1><span>CHOOSE YOUR</span> <b>REWARD</b></h1><p>Choose one booster. The other two return to the vault.</p></header><section class="kotr-reward-king"><span>REIGNING KING</span><strong>${name(run.superstarId)}</strong><small>${state.clears} career crown${state.clears===1?'':'s'}</small></section><div class="kotr-reward-grid">${choiceMarkup}</div></section>`;
    root.querySelectorAll('[data-kotr-reward-set]').forEach(btn=>btn.addEventListener('click',()=>{
      try {
        ...
        grantBooster(profile, 1, setId);
        saveProfile(profile);
        message = `${sets[setId]?.displayName ?? sets[setId]?.name ?? setId} booster claimed. ${name(run.superstarId)} remains the reigning King of the Ring.`;
        renderKingOfTheRing();
      } catch (error) { message = error.message; renderKingOfTheRing(); }
    }));
    return;
  ...
  if (run?.status === "cleared" && run.rewardClaimedSetId) {
    ...
    root.innerHTML = `<section class="ladder-screen kotr-screen premium-screen compact-mode-run premium-run-screen is-run-setup"><section class="feature-hero kotr-feature single-feature-hero mode-run-hero reigning-king-hero">${modePortraits([run.superstarId],"feature-art")}<div class="feature-shade"></div><div class="feature-copy mode-run-hero-copy">${modeLogoMarkup("king-of-the-ring",true)}<span class="kotr-reigning-label">♛ REIGNING KING</span><h2>${name(run.superstarId)}</h2><p>${claimedSet ? `${sets[claimedSet]?.displayName ?? sets[claimedSet]?.name ?? claimedSet} booster claimed.` : 'Tournament reward already claimed.'}</p></div><div class="ladder-summary mode-run-summary"><div><b>${state.clears}</b><span>Crowns</span></div><div><b>3/3</b><span>Winning Run</span></div><div><b>♛</b><span>Reigning</span></div><div><b>8</b><span>Field</span></div></div></section>${message?`<div class="mode-run-status has-message"><span>${message}</span></div>`:''}${bracket}<section class="kotr-post-crown-actions"><button id="kotr-open-packs" class="nav-button">OPEN PACKS</button><button id="start-kotr" class="start-match">ENTER NEW TOURNAMENT</button></section></section>`;
...
function renderChampionship(){
  ...
  const roadNodes = CHAMPIONSHIP_ROAD_OPPONENTS.map((id,index)=>{
    ...
    return `${sectionStart ? `<div class="champ-road-theme theme-${sectionStart.accent}"><span>${sectionStart.label}</span>${setLogoMarkup(sectionStart.setId,'champ-road-set-logo')}<small>MATCHES ${sectionStart.start}–${sectionStart.end}</small></div>` : ''}<article class="champ-road-node ${stateClass} theme-${section.accent}" data-champ-node="${nodeNumber}"><span class="champ-road-line" aria-hidden="true"></span><button type="button" ${active&&index===run.stage?'id="continue-championship"':''} ${active&&index===run.stage?'':'disabled'}><b>${nodeNumber}</b><span class="champ-road-star">${portraitMarkup(id,star?.name??id)}</span><span class="champ-road-copy"><small>${stateClass==='cleared'?'DEFEATED':stateClass==='current'?'NEXT MATCH':'LOCKED'}</small><strong>${star?.name??id}</strong><em>${section.label}</em></span><i>${stateClass==='cleared'?'✓':'★'}</i></button></article>`;
  }).join('');
...
function cardArtFace(card) {
  ...
  if (layered) {
    ...
    return `<img loading="lazy" decoding="async" class="ccg-layered-card-plate" src="${layered}" alt="${card.name}" data-flat-finished-art="${finished ?? ""}" data-legacy-finished-art="${legacyFinished ?? ""}" onerror="const c=this.closest('.ccg-card');const o=c?.querySelector('.ccg-live-front-data');if(o)o.style.display='none';if(!this.dataset.flatTried&&this.dataset.flatFinishedArt){this.dataset.flatTried='1';this.src=this.dataset.flatFinishedArt;return;}if(!this.dataset.legacyTried&&this.dataset.legacyFinishedArt){this.dataset.legacyTried='1';this.src=this.dataset.legacyFinishedArt;return;}this.onerror=null;this.style.display='none';c?.classList.remove('is-layered-front','is-full-art-finished','is-full-art-move');c?.classList.add('uses-rules-fallback','is-flipped');">`;
  }
...
function handleCompletedMatch() {
  ...
  if (activeMode === "king-of-the-ring") {
    ...
    if (outcome.status === "advance") message = `${KING_OF_THE_RING_ROUNDS[run.stage - 1]} won! One step closer to the crown. Next: ${KING_OF_THE_RING_ROUNDS[run.stage]}.`;
      ...
      message = `KING OF THE RING! ${state.players[HUMAN].superstar.name} won three straight matches. The coronation and choose-one booster reward await.`;
    }
  ...
  if (activeMode === "championship") {
    ...
    const checkpointSetId = outcome.sectionCleared?.setId ?? null;
    if (result === "win" && checkpointSetId) grantBooster(profile, 1, checkpointSetId);
    if (outcome.status === "advance") {
      const checkpointText = outcome.sectionCleared ? ` ${outcome.sectionCleared.label} complete — +1 ${setCollections[checkpointSetId]?.displayName ?? setCollections[checkpointSetId]?.name ?? 'set'} booster.` : '';
      ...
      if (outcome.firstWithSuperstar) { grantBooster(profile, 1, checkpointSetId ?? "summerslam-series-1"); message = `CHAMPIONSHIP ROAD CLEARED on ${CHAMPIONSHIP_DIFFICULTIES[run.difficultyId]?.label ?? run.difficultyId}! Final region complete, Championship Pack awarded, plus a first-road bonus booster for ${superstarById[run.superstarId].name}.${outcome.unlockedDifficulty ? ` ${CHAMPIONSHIP_DIFFICULTIES[outcome.unlockedDifficulty].label} is now unlocked.` : ''}`; }
      else message = `CHAMPIONSHIP ROAD CLEARED on ${CHAMPIONSHIP_DIFFICULTIES[run.difficultyId]?.label ?? run.difficultyId}! Final region complete and a Championship Pack has been awarded.${outcome.unlockedDifficulty ? ` ${CHAMPIONSHIP_DIFFICULTIES[outcome.unlockedDifficulty].label} is now unlocked.` : ''}`;
    }
    message += seasonXpText;
    refreshCareerAchievements(profile);
    saveProfile(profile);
  ...
  if (activeMode !== "ladder") {
    if (state.winner === HUMAN) {
      const rewardSetId = isPlayerReleasedSetId(state.players[HUMAN].superstar.setId) ? state.players[HUMAN].superstar.setId : "summerslam-series-1";
  ...
  else if (outcome.status === "cleared") message = `MONEY IN THE BANK CLEARED! Today’s 8-Superstar tower is complete and a Completion Pack has been awarded.`;
  message += seasonXpText;
  refreshCareerAchievements(profile);
  saveProfile(profile);
}
...
function renderMatchResults() {
  ...
  const rewardSetId = isLiveEvent ? (liveRun?.rewardSetId ?? liveTower?.event.rewardSetId ?? "summerslam-series-1") : (isPlayerReleasedSetId(humanStar.setId) ? humanStar.setId : "summerslam-series-1");
  ...
  if (isKingOfTheRing) {
    ...
    rewardLine = tournamentWon ? `Crown secured · choose 1 of 3 released-set boosters after the coronation` : humanWon ? `${KING_OF_THE_RING_ROUNDS[Math.max(0,(kotrRun?.stage ?? 1)-1)]} cleared · no round reward` : "Eliminated · enter a new tournament";
    rewardMarkup = tournamentWon ? `<div class="results-crown-reward"><span>♛</span><strong>${rewardLine}</strong><small>THE KING OF THE RING CORONATION IS NEXT</small></div>` : `<strong>${rewardLine}</strong>`;
    showRewardButton = false;
  } else if (isLadder) {
    const ladderRun = ladderState(profile,new Date()).activeRun;
    const ladderCleared = humanWon && ladderRun?.status === "cleared";
    rewardLine = ladderCleared ? "Money in the Bank Completion Pack awarded" : humanWon ? `Level ${ladderRun?.rung ?? 0}/${LADDER_LENGTH} cleared · no level reward` : `${ladderRun?.lives ?? 0} lives remaining`;
    rewardMarkup = ladderCleared ? `<div class="results-pack-reward">${physicalBoosterPackMarkup({setId:"summerslam-series-1",title:"REWARD",series:"MONEY IN THE BANK",subtitle:"COMPLETION PACK",extraClass:"results-booster"})}<strong>${rewardLine}</strong></div>` : `<strong>${rewardLine}</strong>`;
    showRewardButton = ladderCleared;
  } else if (isLiveEvent) {
