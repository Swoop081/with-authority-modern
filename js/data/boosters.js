...
import { addOwnedCard, unlockSuperstar, ownedCount } from "./profile.js";
...
export function boosterEligible(profile, card, foil, setId = card?.setId ?? DEFAULT_BOOSTER_SET_ID) {
  ...
  if(card.kind==="entrance"){if(!foil)return false;return !!profile?.unlockedSuperstars?.includes(card.superstarId);}
  return true;
}
...
function recordPull(profile,card,foil){const before={normal:ownedCount(profile,card.id,"normal"),foil:ownedCount(profile,card.id,"foil")};const result=addOwnedCard(profile,card.id,{foil});let superstarUnlocked=false;if(card.kind==="superstar"&&!profile.unlockedSuperstars.includes(card.superstarId)){unlockSuperstar(profile,card.superstarId);superstarUnlocked=true;profile.packsSinceSuperstarUnlockBySet??={};profile.packsSinceSuperstarUnlockBySet[card.setId]=0;if(card.setId===DEFAULT_BOOSTER_SET_ID)profile.packsSinceSuperstarUnlock=0;}return{card,foil,isNewFinish:before[foil?"foil":"normal"]===0,superstarUnlocked,replacedNormal:!!result.replacedNormal};}
...
export function openLadderCompletionPack(profile,rng=Math.random,setId=null){
  ...
  const foilSlot=Math.min(BOOSTER_SIZE-1,Math.floor(rng()*BOOSTER_SIZE)),vrSlot=Math.min(BOOSTER_SIZE-1,Math.floor(rng()*BOOSTER_SIZE));const lockedSuperstars=cardsForSet(actualSet).filter(c=>c.kind==="superstar"&&!profile.unlockedSuperstars.includes(c.superstarId)&&boosterEligible(profile,c,false,actualSet));const pending=ladder.firstClearSuperstarPendingBySet?.[actualSet]??(actualSet===DEFAULT_BOOSTER_SET_ID&&ladder.firstClearSuperstarPending);const guaranteedSuperstar=pending&&lockedSuperstars.length?lockedSuperstars[Math.floor(rng()*lockedSuperstars.length)]:null;const guaranteedSlot=guaranteedSuperstar?vrSlot:-1;const pulls=[];
  for(let i=0;i<BOOSTER_SIZE;i++){const preferredFoil=i===foilSlot;let pull;if(i===guaranteedSlot){pull=boosterEligible(profile,guaranteedSuperstar,preferredFoil,actualSet)?{card:guaranteedSuperstar,foil:preferredFoil}:choosePull(profile,actualSet,preferredFoil,rng,4);}else pull=choosePull(profile,actualSet,preferredFoil,rng,i===vrSlot?4:null);pulls.push(recordPull(profile,pull.card,pull.foil));}if(guaranteedSuperstar)consumeFirstClearSuperstarGuarantee(profile,actualSet);return pulls;
}
