...
export function canPlaySpecial(state,playerId,card){ const p=state.players[playerId]; if(!p||p.specialUsed||card?.kind!=="special")return false; if(card?.special?.type==="brassKnuckles")return state.phase==="POST_MOVE"&&state.playerInControl===playerId&&!!p.events?.brassKnucklesWindow; return false; }
export function moveEligibility(state,playerId,card){
 ...
 if(state.phase!=="ACTION")return fail("Not an Action window"); if(state.playerInControl!==playerId)return fail("Not in Control"); if(card?.kind!=="move"||card.defensiveOnly)return fail("Not an offensive Move");
...
export function counterEligibility(state,playerId,incoming,counter){
 ...
 if(!canCounter(incoming,counter))return fail("Move does not Counter this Move Type");
...
export function canAttemptPin(state,playerId){ return state.phase==="POST_MOVE"&&state.playerInControl===playerId?{legal:true,cost:0}:{legal:false,cost:0}; }
