...
export function moveEligibility(state,playerId,card){
 ...
 const sequenceDiscount=(card.discountAfterCounter&&p.events?.counteredThisControl)?card.discountAfterCounter:0; const finisherDiscount=card.finisher?Math.max(0,p.events?.nextFinisherDiscount??0):0; const discount=finisherDiscount+(p.nextMoveDiscount??0)+(p.methodDiscount?.[card.method]??0)+(p.namedDiscount?.[card.name]??0)+sequenceDiscount; const needed=Math.max(0,(card.cost??0)-discount); if(totalMomentum(p)<needed)return fail(`Need ${needed} total Momentum`);
...
export function counterEligibility(state,playerId,incoming,counter){
 ...
 const counterDiscount=Math.max(0,p?.events?.nextCounterDiscount??0); const needed=Math.max(0,(counter.cost??0)-counterDiscount); if(totalMomentum(p)<needed)return fail(`Need ${needed} total Momentum`);
